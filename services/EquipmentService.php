...
class EquipmentService
{
    ...
    public function equipment($status = '', $lenderId = '', $categoryId = '')
    {
        ...
        if ($status === 'out') {
            $conditions[] = 'equipment.status = 1';
        }
        if ($status === 'in') {
            $conditions[] = 'equipment.status = 0';
        }
        if ($status === 'maintenance') {
            $conditions[] = 'equipment.status = 2';
        }
        if ($lenderId !== '') {
        ...
        return $this->db->createCommand(
            ...
            . ' ON open_loan.equipment_id = equipment.id AND open_loan.returned_at IS NULL'
            . ' LEFT JOIN borrower AS open_lender ON open_lender.id = open_loan.borrower_id'
            . $where
    ...
    public function report()
    {
        ...
        return [
            'total' => (int) $this->db
                ->createCommand('SELECT COUNT(*) FROM equipment WHERE status <> 3')
                ->queryScalar(),
            'available' => (int) $this->db
                ->createCommand('SELECT COUNT(*) FROM equipment WHERE status = 0')
                ->queryScalar(),
            'lended' => (int) $this->db
                ->createCommand('SELECT COUNT(*) FROM loan WHERE returned_at IS NULL')
                ->queryScalar(),
            'maintenance' => (int) $this->db
                ->createCommand('SELECT COUNT(*) FROM equipment WHERE status = 2')
                ->queryScalar(),
            'overdue' => (int) $this->db
                ->createCommand(
                    'SELECT COUNT(*) FROM loan WHERE returned_at IS NULL AND due_at < :today',
                    [':today' => $today]
                )
                ->queryScalar(),
            'dueToday' => (int) $this->db
                ->createCommand(
                    'SELECT COUNT(*) FROM loan WHERE returned_at IS NULL AND due_at = :today',
                    [':today' => $today]
                )
    ...
    {
        return $this->db->createCommand(
            'SELECT id, full_name FROM borrower WHERE is_active = 1 ORDER BY full_name'
        )->queryAll();
    }

    public function categories()
    {
        return $this->db->createCommand('SELECT id, name FROM category ORDER BY name')->queryAll();
    }

    public function categoriesWithUsage()
    {
        return $this->db->createCommand(
            'SELECT category.id, category.name, COUNT(equipment.id) AS equipment_count'
            . ' FROM category'
    ...
    public function overdueLoans($filters = [])
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            $rows[$index]['late_fee'] = $deposit > 0
                ? min($daysLate * self::LATE_FEE_PER_DAY, $deposit)
    ...
    public function handleAction($post)
    {
        ...
        $equipment = $this->db
            ->createCommand('SELECT * FROM equipment WHERE id = :id', [':id' => $equipmentId])
            ->queryOne();

        if (!$equipment) {
            return ['success' => false, 'message' => 'Az eszköz nem található.'];
        }

        if ($action === 'return') {
            $this->db->transaction(function () use ($equipmentId) {
                $this->db->createCommand(
                    'UPDATE loan SET returned_at = :today'
                    . ' WHERE equipment_id = :id AND returned_at IS NULL',
                    [':id' => $equipmentId, ':today' => date('Y-m-d')]
                )->execute();
                $this->db->createCommand()
                    ->update('equipment', ['status' => 0], 'id = :id', [':id' => $equipmentId])
                    ->execute();
            });
        ...
        if ($action === 'maintenance') {
            if ($this->isOut($equipmentId)) {
                return [
                    'success' => false,
                    'message' => 'Kölcsönzés alatt álló eszköz nem küldhető szervizbe.',
                ];
            }

            $this->db->createCommand()
                ->update('equipment', ['status' => 2], 'id = :id', [':id' => $equipmentId])
                ->execute();
        ...
        if ($action === 'maintenance_return') {
            if ((int) $equipment['status'] !== 2) {
                return ['success' => false, 'message' => 'Az eszköz nincs szervizben.'];
            }
            ...
            return [
                'success' => true,
                'message' => 'Az eszköz visszakerült az elérhető eszközök közé.',
            ];
        }
        ...
        if ($action === 'lend') {
            ...
            if (!$lenderId || !$starts || !$due || $starts > $due) {
                return ['success' => false, 'message' => 'Adj meg lendert és érvényes időszakot.'];
            }

            $conflict = $this->db->createCommand(
                ...
                [':id' => $equipmentId, ':starts' => $starts, ':due' => $due]
            )->queryOne();

            if ($conflict) {
                return [
                    'success' => false,
                    'message' => 'Ütközés: már ' . $conflict['full_name'] . ' használja '
                        . $conflict['loaned_at'] . ' és ' . $conflict['due_at'] . ' között.',
                ];
            }

            if ((int) $equipment['status'] !== 0) {
                return ['success' => false, 'message' => 'Az eszköz jelenleg nem elérhető.'];
            }
            ...
            $this->db->transaction(function () use ($equipmentId, $lenderId, $starts, $due) {
                ...
                $this->db->createCommand()
                    ->update('equipment', ['status' => 1], 'id = :id', [':id' => $equipmentId])
                    ->execute();
            });
    ...
    private function createEquipment($post)
    {
        ...
        if ($inventoryNo === '' || $name === '' || !$categoryId) {
            return [
                'success' => false,
                'message' => 'Add meg a leltári számot, az eszköz nevét és a kategóriát.',
            ];
        }

        if (
            !$this->db
                ->createCommand('SELECT 1 FROM category WHERE id = :id', [':id' => $categoryId])
                ->queryScalar()
        ) {
            return ['success' => false, 'message' => 'A kiválasztott kategória nem található.'];
        }
        ...
        if (
            ...
            return [
                'success' => false,
                'message' => 'Ez a leltári szám már foglalt: ' . $inventoryNo,
            ];
        }
        ...
        return [
            'success' => true,
            'message' => 'Az eszköz felvéve a leltárba: ' . $inventoryNo . ' - ' . $name,
        ];
    }
    ...
    private function deleteEquipment($equipmentId)
    {
        ...
        if ($this->isOut($equipmentId)) {
            return ['success' => false, 'message' => 'Kölcsönzés alatt álló eszköz nem törölhető.'];
        }

        if (
            $this->db
                ->createCommand('SELECT 1 FROM loan WHERE equipment_id = :id', [':id' => $equipmentId])
                ->queryScalar()
        ) {
            if ((int) $equipment['status'] === 3) {
                return ['success' => false, 'message' => 'Az eszköz már selejt státuszban van.'];
            }
            ...
            return [
                'success' => true,
                'message' => 'Az eszközt már kölcsönözték, ezért törlés helyett selejt státuszba került.',
            ];
        }
    ...
    {
        $category = $this->db
            ->createCommand('SELECT * FROM category WHERE id = :id', [':id' => $categoryId])
            ->queryOne();

        if (!$category) {
            return ['success' => false, 'message' => 'A kategória nem található.'];
        }

        if (
            $this->db
                ->createCommand('SELECT 1 FROM equipment WHERE category_id = :id', [':id' => $categoryId])
                ->queryScalar()
        ) {
            return [
                'success' => false,
                'message' => 'A kategória nem törölhető, amíg eszköz tartozik hozzá.',
            ];
        }
