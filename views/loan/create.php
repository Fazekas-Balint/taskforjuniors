...
<?= $form->field($model, 'equipment_id')->dropDownList(
    ArrayHelper::map($equipmentOptions, 'id', function (Equipment $item) {
        return $item->inventory_no . ' - ' . $item->name;
    }),
    ['prompt' => 'Válasszon eszközt']
) ?>
<?= $form->field($model, 'borrower_id')->dropDownList(
    ArrayHelper::map($borrowerOptions, 'id', 'full_name'),
    ['prompt' => 'Válasszon kölcsönvevőt']
) ?>
